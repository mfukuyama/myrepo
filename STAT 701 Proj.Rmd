...
pacman::p_load(tidyverse, dplyr, ggplot2, data.table, lubridate, glmnet, car, zoo, quantmod, BIS, tidyquant, plotly, timetk, VIM)
```
...
is.infinite(rowSums(as.matrix(data.train.q)))

data.train.q2 <- data.train.q[is.finite(rowSums(as.matrix(data.train.q))),]
data.train.q3 <- data.train.q2[is.nan(rowSums(as.matrix(data.train.q2))),]
...
lm.input <- as.formula(paste("log_total_death_per100k", "~", paste(var.1se, collapse = "+")))
fit.1se.lm <- lm(lm.input, data=combined_dataset2) 
summary(fit.1se.lm)
```
