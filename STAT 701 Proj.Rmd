...
pacman::p_load(tidyverse, dplyr, ggplot2, data.table, lubridate, glmnet, car, zoo, quantmod, BIS, tidyquant, plotly, timetk)
```
...
# Column removal
cols <- grep("QoQ*", colnames(final_full_report))
data_lasso_q <- final_full_report[,..cols]
...
Y <- data.train.q$QoQ_Pct_Growth_Stock_Price
fit.lasso.0 <- cv.glmnet(X,Y, alpha=1, nfolds = 10, family = "binomial", type.measure = "deviance")
plot(fit.lasso.0)
```
