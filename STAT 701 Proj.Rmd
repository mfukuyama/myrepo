...
scratch
```{r load data}
apple <- data.table::fread("AAPL.csv", header = T)
head(apple, 30)


combined_transposed <- combined_table2 %>% 
  gather(key = key, value = value, 2:ncol(combined_table2)) %>% 
  spread(key = names(combined_table2)[1], value = "value")
combined_transposed
...
combined_transposed <- transpose(ttt, keep.names = "col", make.names = "Date")
combined_transposed

sapply(unique(colnames(A)), function(x) rowSums(A[,grepl(x, colnames(A))]))

combined_transposed <- combined_transposed[,colSums(is.na(combined_transposed))<nrow(combined_transposed)]
combined_transposed


filenames <- list.files(pattern="*.csv")
test_table <- lapply(filenames, read.csv)
data <- rbindlist(test_table, fill = TRUE)
t_test_table


ttt <- Reduce(function(x, y) full_join(x, y, by="Date"), test_table)
ttt

combined_transposed <- as.data.frame(t(ttt))
...
Reduce(function(x, y) full_join(x, y, by="Date"), la)
la
...
filenames <- list.files(pattern="*.csv")
data_read <- function(filenames) {
  df <- read.csv(filenames, header = T, na.strings=c(""," ","NA"))
  dft <- t(df)
  return(dft)
}

result <- do.call(rbind, lapply(filenames, data_read))
result


dft <- do.call(rbind, lapply(list.files(pattern = "*.csv"), readCalc))
dft

dtt <- lapply(filenames, function(x) cbind(stock_ticker=strsplit(x,'\\.')[[1]][1], read.csv(x)))
dtt

tttt  %>%
    extract(File, "Ticker_Symbol", remove = FALSE) %>%
...
combined_table <-
    ...
    map_df(readAddFilename)


industry <- data.table::fread("/Users/micha/Downloads/SP500_2017.csv")
jtt <- left_join(btt, industry, by="Ticker_Symbol")
jtt
```

deprecated
```{r combine and transpose}
filenames <- list.files(pattern="*.csv")
#extract stock ticker from file name and add it as column
dtt <- lapply(filenames, function(x) cbind(Ticker_Symbol=strsplit(x,'\\.')[[1]][1], read.csv(x)))

merged_table <- Reduce(function(x, y) merge(x, y, by="Ticker_Symbol"), dtt)

dtt
btt <- Reduce(function(x, y) full_join(x, y, by="Date"), dtt)

#bring in industry - not currently working because industry spreadsheet only joins with first column instance of ticker symbol. Solution would be if you could join industry with each element of list created in above step (dtt) BEFORE joining all of them together in btt. Or keep ticker symbol as column
industry <- data.table::fread("/Users/micha/Downloads/SP500_2017.csv")
jtt <- left_join(btt, industry, by="Ticker_Symbol")

#transpose table for easier analysis later on
combined_transposed <- transpose(jtt, keep.names = "col", make.names = "Date")
combined_transposed
```
...
transpose dataframe
```{r transpose}
names(combined_table2)[1] <- "date"

stringcut <- str_match(combined_table2$date, "X\\s*(.*?)\\s*.x")
combined_table2$date <- stringcut[,2]

combined_table2$date <- gsub("\\.", "-", combined_table2$date)

combined_table2$date <- as.yearmon(combined_table2$date)

names(combined_table2) <- gsub(" ", "_", names(combined_table2))

names(combined_table2) <- gsub("-", "_", names(combined_table2))

names(combined_table2) <- gsub("&", "", names(combined_table2))
